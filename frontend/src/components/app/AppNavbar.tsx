...
export default function AppNavbar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-40 border-b border-line-soft bg-background/90 backdrop-blur-xl">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div ref={searchRef} className="relative hidden sm:block">
          <input
            ...
            className="w-56 px-3 py-1.5 rounded-lg bg-white/5 border border-line text-foreground placeholder:text-muted-2 text-xs focus:outline-none focus:border-[#00C853]/50 transition-colors"
          />
          {showSearch && (searchQuery.trim() || searchResults.length > 0) && (
            <div className="absolute top-full left-0 right-0 mt-1 rounded-lg bg-surface border border-line shadow-xl overflow-hidden">
              ...
              {!searching && searchResults.length === 0 && searchQuery.trim() && (
                <p className="text-xs text-muted-2 text-center py-4">No users found</p>
              )}
              {searchResults.map((u) => (
                <Link key={u.id} href={`/profile/${u.username}`} onClick={() => { setShowSearch(false); setSearchQuery(""); }}
                  ...
                  <div className="w-8 h-8 rounded-full bg-white/10 overflow-hidden flex-shrink-0">
                    {u.avatar ? <img src={u.avatar} alt="" className="w-full h-full object-cover" /> : (
                      <div className="w-full h-full flex items-center justify-center text-xs font-bold text-muted-2">{u.username[0].toUpperCase()}</div>
                    )}
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-foreground truncate">{u.displayName || u.username}</p>
        ...
        <div className="flex items-center gap-5">
          ...
          <Link href={user ? `/profile/${user.username}` : "/login"} className="text-muted-2 hover:text-foreground transition-colors">
            {user?.avatar ? (
